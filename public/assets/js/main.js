...
class TrafficEngine {
    ...
    static async mockFetch(url, options = {}) {
        ...
        if (path === '/model-comparison') {
            const metrics = {
                "Linear Regression": { accuracy: 0.24, rmse: 20300, mae: 14800, r2: 0.24 },
            ...
            const results = Object.entries(metrics).map(([model, m]) => {
                ...
                return {
                    ...
                    metrics: m,
                    is_best: model === "ARIMA (Time-Series)"
                };
            });
            ...
            return {
                json: async () => ({
                    ...
                    results: results,
                    feature_importance: [
                        { feature: "Hour of Day", importance: 0.52 },
                        { feature: "Area Traffic Density", importance: 0.18 },
                        { feature: "Temporal Period (Sin)", importance: 0.13 },
                        { feature: "Temporal Period (Cos)", importance: 0.13 },
                        { feature: "Road Infrastructure", importance: 0.02 },
                        { feature: "Weather / Environment", importance: 0.01 },
                        { feature: "Events / Roadwork", importance: 0.01 }
                    ],
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            ...
            const data = {
                ...
                ml_data: {
                    last_prediction: state.lastPrediction || null,
                    last_simulation: state.lastSimulation || null,
                    last_lab_result: state.lastLabResult || null
                },
                charts: {
                    heatmap: window.heatmapChartInstance ? window.heatmapChartInstance.toBase64Image() : null,
                    weekly: window.weeklyChartInstance ? window.weeklyChartInstance.toBase64Image() : null,
                    comparison: window.comparisonChartInstance ? window.comparisonChartInstance.toBase64Image() : null
                }
            };
    ...
    async function initDashboard() {
        try {
            ...
            if (statusData.active_alerts.length > 0) {
                ...
                // Add permanent Bangalore context alert removed to prevent duplication - handled by backend
                // alertList.innerHTML = '';
    ...
    if (predictForm) {
        predictForm.addEventListener('submit', async e => {
            ...
            try {
                ...
                window.appState.lastPrediction = data;
    ...
    if (simForm) {
        simForm.addEventListener('submit', async e => {
            ...
            try {
                const res = await TrafficEngine.apiFetch('/api/simulation', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        departure_time: departureTime,
                        weather: document.getElementById('simWeather').value === 'rain' ? 'Rain' : 'Clear'
                    })
                });
                const data = await res.json();
                window.appState = window.appState || {};
                window.appState.lastSimulation = data;
    ...
    async function loadInsights() {
        try {
            const res = await TrafficEngine.apiFetch('/api/insights');
            const data = await res.json();
    ...
    async function runBenchmark() {
        ...
        try {
            const res = await TrafficEngine.apiFetch('/api/model-comparison', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            window.appState = window.appState || {};
            window.appState.lastLabResult = data;
        } catch (err) {
